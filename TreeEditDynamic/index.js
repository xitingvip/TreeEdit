...
export default class TreeEdit extends Component {
  constructor(props) {
    super(props);
    let treeData = this.formatData(props.treeList);
    this.state = {
      treeData: treeData,
      expandedKeys: [],
    };
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqual(this.props.treeList, nextProps.treeList)) {
      let treeData = this.formatData(nextProps.treeList);
  ...
  handleTreeData = (treeData, key, type) => {
    treeData.map((mItem, mIndex) => {
      ...
      if (String(mItem.key) === String(key)) {
        ...
        if (type === 'add') {
          if (Number(mItem.level) === 3) {  //三级添加三级
            treeData.push({ title: <TreeInput blurInput={(value) => this.blurInput(value, mItem, 'add')}/>, key: -1 });
          } else {
            mItem.children = [...mItem.children, { title: <TreeInput blurInput={(value) => this.blurInput(value, mItem, 'add')}/>, key: -1 }];
          }
        }
        // 编辑节点，若节点编辑为''，则变为编辑前的内容，变为input框
        if (type === 'edit') {
          mItem.title = <TreeInput value={mItem.copyTitle} key={mItem.key} blurInput={(value) => this.blurInput(value, mItem, 'edit')}/>;
        }
  ...
  addNode = (key) => {
    if (key === null) { //添加一级
      ...
      treeData.push({ title: <TreeInput blurInput={(value) => this.blurInput(value, item, 'add')}/>, key: -1 });
      this.setState({ treeData });
    } else {  // 添加二级或三级
  ...
  render() {
    ...
    return (<div className={styles.body}>
      <Button type="dashed" className={styles['add-btn']} onClick={() => this.addNode(null)}>+ 添加</Button>
      <Tree
        showLine
