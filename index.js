...
const treeData = [
  {
    ...
    id: 2,
    children: [
      { title: '0-1-0-0', copyTitle: '0-1-0-0', parentId: 5, id: 21, key: '0-1-0-0', isEdit: false, selected: false, children: [] },
      { title: '0-1-0-1', copyTitle: '0-1-0-1', parentId: 5, id: 22, key: '0-1-0-1', isEdit: false, selected: false, children: [] },
      { title: '0-1-0-2', copyTitle: '0-1-0-2', parentId: 5, id: 23, key: '0-1-0-2', isEdit: false, selected: false, children: [] },
    ],
  }, {
    title: '0-2',
    copyTitle: '0-2',
...
export default class TreeEdit extends Component {
      ...
      if (String(mItem.key) === String(key)) {
        ...
        if (type === 'add') {
          if (mItem.children && mItem.parentId === -1) { // 一级添加二级
            ...
            let id = idx === 0 ? mItem.id + '1' : Number(mItem.children[idx - 1].id) + 1;
            mItem.children = [...mItem.children, { title: <TreeInput blurInput={(value) => this.blurInput(value, id)}/>, key: id, id, parentId: mItem.id, children: [] }];
          } else if (mItem.children && mItem.parentId !== -1) { // 二级添加三级
            let idx = mItem.children.length;
            let id = idx === 0 ? mItem.id + '1' : Number(mItem.children[idx - 1].id) + 1;
            mItem.children = [...mItem.children, { title: <TreeInput blurInput={(value) => this.blurInput(value, id)}/>, key: id, id, parentId: mItem.id }];
          } else { // 三级添加三级
  ...
  addNode = (key) => {
    if (key === null) { //添加一级
      ...
      let idx = treeData.length;
      let id = Number(treeData[idx - 1].id) + 1;
      treeData.push({ title: <TreeInput blurInput={(value) => this.blurInput(value, id)}/>, key: id, id, parentId: -1, children: [] });
      this.setState({ treeData });
    } else {  // 添加二级或三级
  ...
  render() {
    ...
    return (<div className={styles.body}>
      <Button type="dashed" className={styles['add-btn']} onClick={() => this.addNode(null)}>+ 添加一级节点</Button>
      <Tree
        showLine
